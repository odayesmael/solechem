...
import { Calendar, Clock, ArrowRight, Tag, Search, Newspaper, TrendingUp, Award, Globe } from 'lucide-react';
...
export default function News() {
  ...
  return (
    <div className="min-h-screen bg-white">
        ...
        <div className="max-w-7xl mx-auto px-4 lg:px-8 relative z-20 text-center space-y-8 max-w-4xl mx-auto">
          <div className="inline-flex items-center gap-2 bg-white border border-slate-200 px-3 py-1.5 rounded-sm shadow-sm">
            <span className="text-slate-600 font-bold text-[10px] uppercase tracking-[0.2em] flex items-center gap-2">
              <Newspaper className="w-3.5 h-3.5 text-orange-600" /> Latest Updates
            </span>
          </div>
          ...
          <p className="text-lg text-slate-600 leading-relaxed max-w-2xl mx-auto font-medium">
            Stay informed with the latest company announcements, industry analysis, product launches, and upcoming events from SoleChem Europe.
          </p>
        </div>
      </section>
      ...
      <section className="py-6 bg-white border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 divide-x divide-slate-100">
            {[
              { icon: Newspaper, label: 'Published Articles', value: `${NEWS_ITEMS.length}+` },
              { icon: Globe, label: 'Markets Covered', value: '50+' },
              { icon: TrendingUp, label: 'Industry Reports', value: '12' },
              { icon: Award, label: 'Years of Expertise', value: '13' },
            ].map((stat, i) => (
          ...
          <div className="max-w-7xl mx-auto px-4 lg:px-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {featuredItems.map((item, i) => (
                <motion.article key={item.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="group relative bg-white rounded-sm border border-slate-200 overflow-hidden hover:border-orange-500 hover:shadow-xl transition-all duration-300">
          ...
          {regularItems.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {regularItems.map((item, i) => (
                <motion.article key={item.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }} className="group bg-white rounded-sm border border-slate-200 overflow-hidden hover:border-orange-500 hover:shadow-lg transition-all duration-300 flex flex-col">
      ...
      {/* Newsletter CTA */}
      <section className="py-20 bg-slate-950 text-center px-4 border-t border-slate-800">
        <div className="max-w-3xl mx-auto space-y-8">
          <h2 className="text-[10px] font-bold text-orange-500 uppercase tracking-[0.3em]">Stay Updated</h2>
          <h3 className="text-3xl md:text-4xl font-sans font-bold text-white tracking-tight">Never Miss an Update</h3>
          <p className="text-[15px] text-slate-400 font-medium leading-relaxed">Subscribe to our newsletter for the latest industry insights, product announcements, and exclusive B2B offers delivered straight to your inbox.</p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 max-w-lg mx-auto">
            <input type="email" placeholder="Enter your business email" className="w-full h-12 bg-slate-800 border border-slate-700 rounded-sm px-4 text-[14px] text-white placeholder:text-slate-500 focus:ring-1 focus:ring-orange-500 focus:border-orange-500 outline-none transition-all" />
            <button className="w-full sm:w-auto bg-orange-600 text-white px-8 h-12 rounded-sm font-semibold text-[12px] uppercase tracking-widest hover:bg-orange-700 transition-colors whitespace-nowrap shrink-0">Subscribe</button>
          </div>
          <p className="text-[11px] text-slate-600 font-medium">No spam. Unsubscribe anytime. B2B content only.</p>
        </div>
      </section>
    </div>
  );
